...
from semi_src.utils import bool_flag, initialize_exp
from semi_src.models import build_model
from semi_src.trainer import Trainer
from semi_src.evaluation import Evaluator
from semi_src.refinement import generate_new_dictionary, symmetric_reweighting
from semi_src.evaluation.word_translation import get_word_translation_accuracy
...
parser.add_argument("--distance_lambda", type=float, default=1, help="keep the distance")
...
parser.add_argument("--batch_size1", type=int, default=8, help="Batch size1")
#parser.add_argument("--batch_size1", type=int, default=1000, help="Batch size1")
...
# Autoencoder
parser.add_argument("--autoenc_epochs", type=int, default=50, help="Number of initail autoencoder epochs")
...
parser.add_argument("--l_relu", type=int, default=1, help="Whether bottleneck of autoenconder use leaky relu activation")
# Mutiple discrimation
parser.add_argument("--muutiple_train", type=bool_flag, default=True, help="Whether train multiple discrimation")
...
src_emb, tgt_emb, mapping_G, mapping_F, discriminator_A, discriminator_B, encoder_A, decoder_A, encoder_B, decoder_B,discriminator_A_Align,discriminator_B_Align = build_model(params, True)
trainer = Trainer(src_emb, tgt_emb, mapping_G, mapping_F, discriminator_A, discriminator_B, encoder_A, decoder_A, encoder_B, decoder_B,discriminator_A_Align,discriminator_B_Align, params)
...
if params.adversarial:
    ...
    src_A = trainer.encoder_A(src_emb.weight.data).data
    tgt_B = trainer.encoder_B(tgt_emb.weight.data).data

    stats = {'MAP_COSTS': []}
    trainer.load_training_dico(params.dico_train)
    trainer.load_training_dico_B(params.dico_train)

    trainer.procrustes_G(stats)
    trainer.procrustes_F(stats)
    ...
    for n_epoch in range(params.n_epochs):
        ...
        for n_iter in tqdm(range(0, params.epoch_size, params.batch_size)):
            ...
            for _ in range(params.dis_steps):
                trainer.dis_step_A_align(stats)
                trainer.dis_step_B_align(stats)

            # mapping training
            n_words_proc_G += trainer.mapping_step_G(stats)
            n_words_proc_F += trainer.mapping_step_F(stats)
            n_words_proc += trainer.mapping_step_rcsls_G(stats)
            n_words_proc += trainer.mapping_step_rcsls_F(stats)
            n_words_proc += trainer.mapping_step_align_G(stats)
            n_words_proc += trainer.mapping_step_align_F(stats)
        ...
        trainer.orthogonalize_G()
        trainer.orthogonalize_F()
        # model evaluation
        ...
        for n_iter in range(params.n_procrustes):#n_procrustes=100
            trainer.procrustes_AB()
            # build a dictionary from aligned embeddings
            emb1 = (trainer.mapping_G(trainer.encoder_A(trainer.src_emb.weight.data)).data)[5000:params.dico_max_rank]
            emb2 = (trainer.encoder_B(trainer.tgt_emb.weight.data).data)[5000:params.dico_max_rank]
            emb1 = emb1 / emb1.norm(2, 1, keepdim=True).expand_as(emb1)
            emb2 = emb2 / emb2.norm(2, 1, keepdim=True).expand_as(emb2)
            all_pairs, all_scores = generate_new_dictionary(emb1, emb2)
            #print("all_pairs:", all_pairs.shape, type(all_pairs), all_pairs)
            all_pairs = all_pairs.cuda()
            all_pairs = torch.cat([trainer.dico,all_pairs],0)
            #print("all_pairs:", all_pairs.shape, type(all_pairs), all_pairs)
        ...
        for n_iter in range(params.n_procrustes):
            ...
            # build a dictionary from aligned embeddings
            emb1 = ((trainer.encoder_A(trainer.src_emb.weight.data)).data)[5000:params.dico_max_rank]
            emb2 = (trainer.mapping_F(trainer.encoder_B(trainer.tgt_emb.weight.data)).data)[5000:params.dico_max_rank]
            emb1 = emb1 / emb1.norm(2, 1, keepdim=True).expand_as(emb1)
            emb2 = emb2 / emb2.norm(2, 1, keepdim=True).expand_as(emb2)
            all_pairs, all_scores = generate_new_dictionary(emb2, emb1)
            all_pairs = all_pairs.cuda()
            all_pairs = torch.cat([trainer.dico_B,all_pairs],0)
        ...
        for i in range(params.n_symmetric_reweighting):
            ...
            emb1 = xw[5000:params.dico_max_rank]
            emb2 = zw[5000:params.dico_max_rank]
            emb1 = emb1 / emb1.norm(2, 1, keepdim=True).expand_as(emb1)
            emb2 = emb2 / emb2.norm(2, 1, keepdim=True).expand_as(emb2)
            all_pairs, all_scores = generate_new_dictionary(emb2, emb1)
            all_pairs = all_pairs.cuda()
